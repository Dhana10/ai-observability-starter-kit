"""Inspect continuous-eval rule + recent runs on Foundry."""
...
def main() -> int:
    ...
    # List rules
    print("=== continuous_evaluation_rules.list() ===")
    rules = []
    try:
        for r in project.beta.continuous_evaluation_rules.list():
            rules.append(_to_json(r))
            print(f"  - id={getattr(r, 'id', '?')} status={getattr(r, 'status', '?')}")
    except Exception as exc:  # noqa: BLE001
        print(f"list error: {type(exc).__name__}: {exc}")

    (ARTIFACTS / "continuous-eval-rules-listed.json").write_text(json.dumps(rules, indent=2))

    # List evals on openai_client
    openai_client = project.get_openai_client()
    cont = json.loads((ARTIFACTS / "continuous-eval.json").read_text())
    eval_id = cont["eval_id"]
    print(f"\n=== evals.retrieve({eval_id}) ===")
    try:
        ev = openai_client.evals.retrieve(eval_id)
        ev_dump = _to_json(ev)
        (ARTIFACTS / "continuous-eval-definition.json").write_text(json.dumps(ev_dump, indent=2))
        print(json.dumps(ev_dump, indent=2)[:1500])
    except Exception as exc:  # noqa: BLE001
        print(f"retrieve error: {type(exc).__name__}: {exc}")

    print(f"\n=== evals.runs.list({eval_id}) ===")
    try:
        runs = list(openai_client.evals.runs.list(eval_id=eval_id))
        print(f"  run_count={len(runs)}")
        for r in runs[:5]:
            print(f"  - {getattr(r,'id','?')} status={getattr(r,'status','?')} created={getattr(r,'created_at','?')}")
    except Exception as exc:  # noqa: BLE001
        print(f"runs list error: {type(exc).__name__}: {exc}")
